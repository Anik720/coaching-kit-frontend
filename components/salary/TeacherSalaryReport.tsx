...
export default function TeacherSalaryReport() {
  ...
  const loadSalaries = (month?: string) => {
    dispatch(fetchSalaries({ userType: 'teacher', page: 1, limit: 200, month }));
  };
  ...
  // Filter out orphan records (deleted teacher — user is null)
  ...
  const handleCleanup = async () => {
    ...
    if (!confirm(`${orphanCount}টি orphan salary record permanently delete হবে (deleted teacher-দের)। Continue?`)) return;
    setCleaningUp(true);
    try {
  ...
  return (
    <div className={styles.pageContainer}>
      ...
      <div style={{
        ...
        <div>
          <h1 style={{ fontSize: 22, fontWeight: 700, color: '#111827', margin: 0 }}>Teacher Salary Report</h1>
          <p style={{ color: '#6b7280', fontSize: 14, margin: '4px 0 0' }}>View and manage all teacher salary payments</p>
        </div>
