...
export default function EditStaffPage({ id }: { id: string }) {
  const router = useRouter();
  const { currentEmployee, loading, dispatch, update, fetchById } = useEmployee();
  ...
  useEffect(() => {
    if (currentEmployee) {
      setForm({
        ...
        motherName: currentEmployee.motherName || "",
        religion: currentEmployee.religion || "islam",
        gender: currentEmployee.gender || "",
        dateOfBirth: currentEmployee.dateOfBirth ? String(currentEmployee.dateOfBirth).split('T')[0] : "",
        contactNumber: currentEmployee.contactNumber || "",
        emergencyContactNumber: currentEmployee.emergencyContactNumber || "",
        presentAddress: currentEmployee.presentAddress || "",
        permanentAddress: currentEmployee.permanentAddress || "",
        whatsappNumber: currentEmployee.whatsappNumber || "",
        email: currentEmployee.email || "",
        secondaryEmail: currentEmployee.secondaryEmail || "",
        nationalId: currentEmployee.nationalId || "",
        bloodGroup: currentEmployee.bloodGroup || "",
        systemEmail: currentEmployee.systemEmail || "",
        password: "",
        confirmPassword: "",
        designation: currentEmployee.designation || "",
        salary: currentEmployee.salary ? String(currentEmployee.salary) : "",
        joiningDate: currentEmployee.joiningDate ? String(currentEmployee.joiningDate).split('T')[0] : "",
      });
      if (currentEmployee.profilePicture) {
        setProfilePreview(`${API_BASE}${currentEmployee.profilePicture}`);
      }
    }
  }, [currentEmployee]);
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    const tid = toastManager.showLoading("Updating staff profile...");
    ...
    try {
      ...
      const textFields: (keyof FormState)[] = [
        ...
        "systemEmail", "designation", "salary", "joiningDate",
      ];

      textFields.forEach((key) => {
        const val = form[key];
        if (val !== undefined && val !== null) {
          formData.append(key, typeof val === 'string' ? val.trim() : String(val));
        }
      });
      ...
      if (result?.type?.endsWith('/rejected')) {
        // @ts-ignore
        throw new Error(result.payload || "Failed to update staff");
      }

      toastManager.safeUpdateToast(tid, "Staff profile updated successfully!", "success");
      router.push("/dashboard/employee/list");
    } catch (err: any) {
      toastManager.safeUpdateToast(tid, err.message || "Failed to update staff", "error");
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <div className={styles.pageContainer}>
      ...
      <div className={styles.pageHeader}>
        <div className={styles.headerText}>
          ...
          <p>Update existing employee account information</p>
        </div>
      ...
      {loading && !currentEmployee ? (
        <div style={{ textAlign: "center", padding: "40px" }}>Loading employee data...</div>
      ) : (
        <>
          {/* Basic Information */}
          ...
          <div className={styles.sectionCard}>
            ...
            <div className={styles.formGrid}>
              <div className={styles.formField}>
                <label className={styles.formLabel}>* Email Address (System Login)</label>
                <input
                  type="text"
              ...
              <div className={styles.formField}>
                <label className={styles.formLabel}>Change Password (leave blank to keep current)</label>
                <input
                  type="password"
          ...
          <div className={styles.sectionCard}>
            ...
            <div className={styles.formActions} style={{ marginTop: 24 }}>
              <button
                ...
                type="button"
              >
                {submitting || loading ? "Updating..." : "✅ Update Staff"}
              </button>
              <Link href="/dashboard/employee/list" className={styles.btnSecondary}>
                📋 Cancel
              </Link>
