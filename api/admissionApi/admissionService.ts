...
class AdmissionService {
  ...
  async createAdmission(admissionData: CreateAdmissionDto): Promise<AdmissionItem> {
    ...
    const formattedData = {
      ...
      fatherMobileNumber: admissionData.fatherMobileNumber || '',
      mothersName: admissionData.mothersName || '',
      guardianMobileNumber: admissionData.localGuardianMobileNumber || '',
      ...
      paymentInstallment: admissionData.paymentInstallment ? Number(admissionData.paymentInstallment) : undefined,
      ...
      status: (admissionData as any).status || AdmissionStatus.PENDING
    };
  ...
  async updateAdmission(registrationId: string, admissionData: UpdateAdmissionDto): Promise<AdmissionItem> {
    ...
    const formData = this.toFormData({
      ...
      father_mobile_number: admissionData.fatherMobileNumber,
      ...
      payment_installment: admissionData.paymentInstallment,
  ...
  // ==========================================
  // Admission Settings API Calls
  // ==========================================
  async getAdmissionSettings(): Promise<AdmissionSetting> {
    const response = await api.get<AdmissionSetting>('/admission-settings');
    return response.data;
  }

  async updateAdmissionSettings(fields: FormFields): Promise<AdmissionSetting> {
    const response = await api.put<AdmissionSetting>('/admission-settings', { fields });
    return response.data;
  }

  // ==========================================
  // NEW: Admission Templates API Calls
  // ==========================================
  async getAdmissionTemplates(): Promise<AdmissionTemplate[]> {
    const response = await api.get<AdmissionTemplate[]>('/admission-templates');
    return response.data;
  }

  async createAdmissionTemplate(data: Partial<AdmissionTemplate>): Promise<AdmissionTemplate> {
    const response = await api.post<AdmissionTemplate>('/admission-templates', data);
    return response.data;
  }

  async updateAdmissionTemplate(id: string, data: Partial<AdmissionTemplate>): Promise<AdmissionTemplate> {
    const response = await api.put<AdmissionTemplate>(`/admission-templates/${id}`, data);
    return response.data;
  }

  async deleteAdmissionTemplate(id: string): Promise<void> {
    await api.delete(`/admission-templates/${id}`);
  }
}
